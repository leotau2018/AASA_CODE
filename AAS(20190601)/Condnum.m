...
function condn=Condnum(k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u,P, n, lambda, zc_lu, zb_l,epsilonk,BBstepsize, z0_lu,g)
% P, lambda,n,k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, zc_lu, zb_l,epsilonk,BBstepsize, z0_lu,g
% function  condn=Condnum2( S, Y, lenS, start_index, gamma, M, P, lambda,n,k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, zc_lu, zb_l,epsilonk,BBstepsize, z0_lu,g,cgtol,cgmaxit,type);
...
% global Atest;Atest=2*diag(length(F):-1:1)+ones(length(F),length(F));cond(Atest,1) %code testing


myfun=@(flag,h)Axfun(flag,h,k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, Omega12,P, n, F);
normA=mynormest(myfun);
myfun=@(flag,h)Ainverse_xfun(flag,h,k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, Omega12,P, n, F);
 normA_inv=mynormest(myfun);
 condn=normA*normA_inv;
% condn=normA;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [est, v, w, iter] = mynormest(A, t, X, varargin)
...
%%
% compute the Marix-vector(matrix) product, reduced A*x,
% decompose each of the t cloumns of the latter matrix(if x is matrix) to be 1 cloumn vector 
function Ax=Axfun(flag,x,  k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, Omega12,P, n, F)
% global Atest;%code testing
if isequal(flag,'dim')
    Ax=length(F);
...
elseif isequal(flag,'notransp')
    for j=1:size(x,2)
        h=zeros(k_e+k_l+k_u,1);
        h(F)=x(:,j);  %size(x,1)=size(F,1)
        Axtmpt=Jacobian_matrix2(k_e,k_l,k_u, h, I_e,J_e, I_l,J_l, I_u, J_u, Omega12,P, n);
        Ax(:,j)=Axtmpt(F);%the output is reduced A*x,size(Ax)==(length(F),sie(x,2))
% Axtmpt=Atest*x(:,j);Ax(:,j)=Axtmpt;%code testing
    end
...
%%
% compute the Marix-vector(matrix) product, reduced A{-1}x
% decompose the t cloumns of the latter matrix to be 1 cloumn vector 
function Ainvx=Ainverse_xfun(flag,x,  k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, Omega12,P, n,F)
tol=1e-6;% global Atest;%code testing
if isequal(flag,'dim')
    Ainvx=length(F);
elseif isequal(flag,'real')
    Ainvx=1;
elseif isequal(flag,'notransp')
    for j=1:size(x,2)
        g=x(:,j); % g is reduced, length(g)=length(F)
        c=ones(k_e+k_l+k_u,1);
        Ainvxtmpt= pre_cg2(g,tol,2000,c(F),F,k_e,k_l,k_u, I_e,J_e, I_l,J_l, I_u, J_u, Omega12,P,n,k_e+k_l+k_u);
% Ainvxtmpt=Atest\g;%code testing
        Ainvx(:,j)=Ainvxtmpt;
    end
elseif isequal(flag,'transp')%暂且不管转置与否
    for j=1:size(x,2)
