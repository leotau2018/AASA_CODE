function [X,z,info,itr,time_used,condn] = AASA_BB_Lbfgs_SNN_adapt(G,ConstrA,OPTIONS,z0)
%AASA_BB_Lbfgs_SNN_adapt，测试的主程序。主要思路：先利用了BB-point的信息，将变量分成积极约束集和非积极约束集，然后返回x_k处迭代。
...
if disp
    fprintf('\n ******************************************************** \n')
    fprintf( '    The AASA(adaptive) method       ')
    fprintf('\n ******************************************************** \n')
...
for itr = 1 : maxit
    ...
    calstep_time0 = clock;
    [ds_e, ds_lu] = getD_byBB_new(S, Y, lr-1, start_index,  gamma, M, P, lambda,n,k_e,k_l,k_u, I_e,J_e, I_l,J_l,I_u, J_u, zc_lu, zb_l,epsilonk,BBstepsize, z0_lu,g,cgtol,cgmaxit,acc_type);%get the search direction at current poin，w是omega.
